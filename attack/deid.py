...
class DeID:
    ...
    def forward_batch(self, images, pallate_boxs):
        """
        Forward batch of images
        """
        deid_images = []
        for (image, plate_box) in zip(images, pallate_boxs):
            deid_images.append(self(image, plate_box))
        return deid_images
...
class Pixelate(DeID):
    ...
    def __call__(self, image, plate_box):
        """
        :params:
            image: cv2 image
